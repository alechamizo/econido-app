...
async function main() {
  try {
    ...
    // Insertar cajas nido
    let nestBoxCount = 0;
    const nestBoxMap = new Map(); // Mapear cajaId → {id (int), uuid}

    // Primero, obtener los IDs existentes
    const existingBoxes = await sql`SELECT id, "cajaId" FROM "nestBoxes"`;
    for (const box of existingBoxes) {
      nestBoxMap.set(box.cajaId, { id: box.id, uuid: randomUUID() });
    }
    ...
    for (const feature of geojson.features) {
      ...
      if (!nestBoxMap.has(cajaId)) {
        try {
          // Usar columnas con camelCase según la estructura real
          const result = await sql.unsafe(`
            INSERT INTO "nestBoxes" ("cajaId", "instalacion", "tipoCaja", "latitude", "longitude", "estadoActual", "ultimaEspecie", "createdAt", "updatedAt")
            VALUES ($1, $2, $3, $4, $5, 'desconocido', NULL, NOW(), NOW())
            RETURNING id
          `, [cajaId, instalacion, tipoCaja, latitude, longitude]);
          
          if (result.length > 0) {
            nestBoxMap.set(cajaId, { id: result[0].id, uuid: randomUUID() });
            nestBoxCount++;
          }
        } catch (err) {
    ...
    console.log(`✅ ${nestBoxCount} cajas nido insertadas (${nestBoxMap.size} total)\n`);
    ...
    for (const line of lines) {
      ...
      const ocupacion = parts[2].trim() === 'Ocupada' ? true : false;
      const psf = parts[4].trim();
      const codigoCN = parts[6].trim();
      const especie = parts[7].trim() || null;
      const numHuevos = parseInt(parts[9]) || 0;
      const numPollos = parseInt(parts[10]) || 0;
      ...
      const nestBoxInfo = nestBoxMap.get(codigoCN);
      if (!nestBoxInfo) {
      ...
      try {
        // Usar columnas snake_case según la estructura real
        const status = ocupacion ? 'occupied' : 'empty';
        await sql.unsafe(`
          INSERT INTO "inspections" ("nestboxid", "userid", "fecha", "ocupada", "especie", "num_huevos", "num_pollos", "observaciones", "status", "created_at")
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
        `, [nestBoxInfo.uuid, '1', fechaISO, ocupacion, especie, numHuevos, numPollos, observaciones, status]);
    ...
    for (const [cajaId, inspection] of inspectionsByBox) {
      const nestBoxInfo = nestBoxMap.get(cajaId);
      if (!nestBoxInfo) continue;

      const estadoActual = inspection.ocupacion ? 'ocupada' : 'vacia';
      const status = inspection.ocupacion ? 'occupied' : 'empty';
      const ultimaEspecie = inspection.especie || 'Sin datos';

      try {
        await sql.unsafe(`
          UPDATE "nestBoxes"
          SET "estadoActual" = $1, "ultimaEspecie" = $2, "updatedAt" = NOW()
          WHERE "id" = $3
        `, [estadoActual, ultimaEspecie, nestBoxInfo.id]);
        updateCount++;
      } catch (err) {
